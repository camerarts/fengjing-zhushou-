...
import { ArrowRight, Sparkles, Layers, Image as ImageIcon, Lock, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen text-slate-100 flex flex-col relative overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-4 z-10 mt-10 md:mt-0 relative">
        ...
        {showPassword ? (
          <div className="w-full max-w-md animate-fade-in bg-slate-900/50 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl">
              <button 
                onClick={() => setShowPassword(false)}
                className="mb-6 text-slate-500 hover:text-white flex items-center gap-2 text-sm transition-colors"
              >
                <ArrowLeft size={16} /> 返回
              </button>

              <h2 className="text-2xl font-bold text-white mb-2">访问验证</h2>
              <p className="text-slate-400 text-sm mb-6">请输入访问密码以进入系统。</p>
              ...
              <form onSubmit={handleSubmitPassword} className="space-y-4">
                  <div className="relative">
                      ...
                      <input 
                        ...
                        placeholder="输入密码"
                        className={`w-full bg-black/40 border rounded-xl px-12 py-4 text-white focus:outline-none transition-all placeholder:text-slate-600 text-center tracking-widest text-lg
                          ${error ? 'border-red-500/50 focus:border-red-500' : 'border-white/10 focus:border-brand-500/50'}`}
                        autoFocus
                      />
                  </div>
                  {error && <p className="text-red-400 text-xs font-bold">密码错误，请重试。</p>}

                  <button 
                    type="submit"
                    className="w-full py-4 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-bold shadow-lg shadow-brand-900/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
                  >
                    进入系统
                  </button>
              </form>
